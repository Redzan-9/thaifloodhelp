...
const InteractiveMap = ({
  ...
  const ensureFloodLayerOnMap = () => {
    ...
    if (!mapRef.current.hasLayer(floodLayerRef.current)) {
      ...
      if (floodLayerRef.current.bringToFront)
  ...
  useEffect(() => {
    ...
    if (showFloodLayer) {
      ...
      if (
        ...
        mapRef.current.hasLayer(floodLayerRef.current)
      ) {
  ...
  useEffect(() => {
    ...
    validReports.forEach((report) => {
      ...
      const urgencyColors =
        URGENCY_COLORS[urgency_level as keyof typeof URGENCY_COLORS] ||
        URGENCY_COLORS[3]
